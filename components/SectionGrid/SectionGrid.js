  ...
  return (
    <>
      <Box className="grid_wrapper" {...rowStyle}>
        {loading ? (
          limits.map((item, i) => (
            <Box className="grid_column" width={columnWidth} key={i} {...columnStyle}>
              {placeholder ? placeholder : <Text content="Loading ..." />}
            </Box>
          ))
        ) : data && data.length > 0 ? (
          data.map((item, index) => (
            <Box className="grid_column" width={columnWidth} key={index} {...columnStyle}>
              <ProductSearchCard query={query} link={link} {...item} />
            </Box>
          ))
        ) : (
          <div>No Available Hotels</div>
        )}
      </Box>
      ...
      {paginationComponent && <Box className="pagination_wrapper">{paginationComponent}</Box>}
    </>
  );
}
