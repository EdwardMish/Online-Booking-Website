...
import ListingWrapper, { PostsWrapper, ShowMapCheckbox } from 'containers/Listing/Listing.style';

const FilterDrawer = dynamic(() => import('containers/Listing/Search/MobileSearchView'));

export default function ListingPage({ processedData, deviceType }) {
  const { state, dispatch } = useContext(SearchContext);
  const statekey = searchStateKeyCheck(state);
  const [posts, setPosts] = useState(processedData.slice(0, LISTING_PAGE_POST_LIMIT) || []);
  ...
  return (
    <ListingWrapper>
      <Head>
        <title>Listing | Reservations Center</title>
      </Head>

      <Sticky top={82} innerZ={999} activeClass="isHeaderSticky">
        <Toolbar
          left={deviceType === 'desktop' ? <CategorySearch /> : <FilterDrawer />}
          right={
            <ShowMapCheckbox>
