...
import { IoLogoTwitter, IoLogoFacebook, IoLogoInstagram, IoIosAdd } from 'react-icons/io';
...
const AgentProfileInfo = (props) => {
  ...
  const { first_name, last_name, content, profile_pic, cover_pic, social_profile } =
    processedData[0];

  return (
    <Fragment>
      <BannerSection>
        <Image src={cover_pic.url} alt="Profile Cover Pic" layout="fill" objectFit="cover" />
      </BannerSection>
      ...
      <UserInfoArea>
        <Container fluid={true}>
          <ProfileImage>
            {profile_pic ? (
              <Image src={profile_pic.url} alt="Profile Pic" layout="fill" objectFit="cover" />
            ) : (
              <ProfilePicLoader />
            )}
          </ProfileImage>
          ...
          <ProfileInformationArea>
            ...
            <SocialAccount>
              <Popover content="Twitter">
                <a href={social_profile.twitter} target="_blank" rel="noopener noreferrer">
                  <IoLogoTwitter className="twitter" />
                </a>
              </Popover>
              <Popover content="Facebook">
                <a href={social_profile.facebook} target="_blank" rel="noopener noreferrer">
                  <IoLogoFacebook className="facebook" />
                </a>
              </Popover>
              <Popover content="Instagram">
                <a href={social_profile.instagram} target="_blank" rel="noopener noreferrer">
